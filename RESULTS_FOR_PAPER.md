...
## 16. RU-granularity sensitivity (scheduler-harness scalability)

Answers the reviewer question "does the cooldown-on-failure result depend on the
number of scheduler-visible RUs or on a wider OFDMA-like channel?". Regenerated by
the scripts in `docs/reproducibility.md` ("RU-granularity sensitivity"); outputs
under `results/ru_granularity/` with a full coordinator write-up in
`results/ru_granularity/RU_GRANULARITY_REPORT.md`.

> **Modeling boundary.** RUs are **abstract scheduler-visible units** with
> seed-controlled per-RU fading; jammer occupancy is a **fraction of RUs**; the
> physical noise bandwidth is held fixed at 20 MHz. This is **not** a calibrated
> IEEE 802.11ax PHY, **not** a full 80/160 MHz validation, and **not** validated
> PHY frequency diversity. Safe terms: "scheduler-harness RU-granularity
> sensitivity", "wider-bandwidth-like OFDMA scheduler abstraction".

- Scripts: `scripts/run_ru_granularity_sweep.py` (180-run minimal matrix, ~9 s),
  `scripts/analyze_ru_granularity.py` (stats + figures + tables + LaTeX).
- Matrix: MCS 3, 128 bit, 3 m, 10 ms deadline, reactive (4/20 ms), AR(1), T_c=5 ms,
  3 seeds; policies S4/baseline, S8/retarget-only, cooldown-only, S9/cooldown+retarget;
  jammer modes broadband (all RUs) / narrowband (1 RU) / partial-band (~50% RUs);
  RU counts {4,8,16,32,64}.
- Findings (from `ru_granularity_summary.csv`):
  - **Broadband:** RU retargeting yields PDR_on = 0 at every RU count; cooldown
    restores PDR_on = 1.0 and deadline-miss 0.252 -> 0 observed (rule-of-three
    UB 2.5e-4) at p95 ~ 5.1 ms — **invariant across R = 4..64**.
  - **Narrowband / partial-band:** retargeting escapes to a clean RU (PDR_on = 1.0,
    already at R = 4); cooldown also recovers.
  - The baseline narrowband PDR_on rise (0.50 -> 0.97 as R grows) is the shrinking
    jammed fraction 1/R, **not** a scheduling gain.
- Conclusion: cooldown is the dominant mechanism under broadband jamming and its
  effect does not depend on RU granularity; RU retargeting helps only when the
  impairment is frequency/RU-selective. Recommended placement: a short Sensitivity
  Analysis subsection (main paper) using `fig_pdron_vs_ru` +
  `ru_granularity_mechanism_table.md`; LaTeX in
  `results/ru_granularity/LATEX_SNIPPETS_RU_GRANULARITY.tex`. The optional extended
  matrix is not required (the minimal result is already flat across RU count).
