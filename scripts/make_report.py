...
def main() -> int:
    ...
    lines += [
        ...
        "",
        "## Anti-Jamming Metric Ranges",
        "",
        f"- SJR (signal - jammer): {metric_range(rows, 'sjr_db')} dB",
        f"- JNR (jammer - noise): {metric_range(rows, 'jnr_db')} dB",
        f"- jammer_duty_cycle: {metric_range(rows, 'jammer_duty_cycle')}",
        f"- PDR | jammer ON: {metric_range(rows, 'pdr_jammer_on')}",
        f"- PDR | jammer OFF: {metric_range(rows, 'pdr_jammer_off')}",
        f"- burst-induced loss ratio: {metric_range(rows, 'burst_induced_loss_ratio')}",
        f"- mean recovery time: {metric_range(rows, 'mean_recovery_time_s')} s",
        f"- robustness ratio (PDR_jam / PDR_clean): {metric_range(rows, 'robustness_ratio')}",
        f"- PLR delta due to jammer: {metric_range(rows, 'plr_increase_due_to_jammer')}",
        "",
        ...
        "- Empty (CSV) / `null` (JSON) values in the anti-jamming columns indicate the metric is not defined for that row (e.g. SJR/JNR on no-jammer baselines, recovery time on constant-jammer runs). Do not coerce to 0 in summary statistics.",
        "",
    ]
