...
CsvHeader()
{
    return {"git_commit",
            ...
            "deadline_miss_due_to_repeated_retry",
            "temporal_gain",
            "ru_diversity_gain",
            "combined_gain",
            "jammer_follow_retry_prob",
            "PDR",
            "PDR_on",
            "PDR_off",
            "PLR",
...
CsvRow(const RunMetrics& m)
{
    return {m.context.gitCommit,
            ...
            m.context.mcsLabel,
            m.context.modulation,
            m.context.codingRate,
            std::to_string(m.context.numRus),
            m.context.perRuChannelEnabled ? "true" : "false",
            ToString(m.context.ruCorrelationRho),
            std::to_string(m.context.ruWidthTones),
            std::to_string(m.context.ruIdInitial),
            std::to_string(m.context.ruIdRetry),
            m.context.ruChanged ? "true" : "false",
            std::to_string(m.context.ruDistanceTones),
            m.context.ruWasJammedInitial ? "true" : "false",
            m.context.ruWasJammedRetry ? "true" : "false",
            ToString(m.context.sinrInitialDb),
            ToString(m.context.sinrRetryDb),
            ToString(m.context.estimatedSinrInitialDb),
            ToString(m.context.estimatedSinrRetryDb),
            ToString(m.context.perInitial),
            ToString(m.context.perRetry),
            ToString(m.context.estimatedPerInitial),
            ToString(m.context.estimatedPerRetry),
            std::to_string(m.context.estimatedBestRu),
            std::to_string(m.context.oracleBestRu),
            ToString(m.context.ruRetargetSuccess),
            std::to_string(m.context.s9CooldownSymbols),
            ToString(m.context.s9CooldownMs),
            ToString(m.context.deadlineS * 1000.0),
            m.context.jammerRuMode,
            ToString(m.context.jammerBurstMs),
            ToString(m.context.jammerIntervalMs),
            ToString(m.context.jammerPhaseMs),
            std::to_string(m.context.jammedRuCount),
            ToString(m.context.fractionRusJammed),
            ToString(m.context.retryLandedAfterBurst),
            ToString(m.context.retryLandedSameBurst),
            ToString(m.context.retryLandedOnJammedRu),
            ToString(m.context.s9SnirNoiseStdDb),
            std::to_string(m.context.s9SnirStalenessSlots),
            ToString(m.context.s9JammerMissedDetProb),
            ToString(m.context.s9JammerFalseAlarmProb),
            ToString(m.context.deadlineMissDueToCooldown),
            ToString(m.context.deadlineMissDueToLoss),
            ToString(m.context.deadlineMissDueToQueueing),
            ToString(m.context.deadlineMissDueToRepeatedRetry),
            ToString(m.context.temporalGain),
            ToString(m.context.ruDiversityGain),
            ToString(m.context.combinedGain),
            ToString(m.context.jammerFollowRetryProb),
            ToString(m.pdr),
            ToString(m.antiJamming.pdrJammerOn),
            ToString(m.antiJamming.pdrJammerOff),
            ToString(m.plr),
            ToString(m.per),
            ToString(m.p95DelayS * 1000.0),
            ToString(m.p99DelayS * 1000.0),
            ToString(m.antiJamming.worstCaseBurstLatencyS * 1000.0),
            ToString(m.antiJamming.meanRecoveryTimeS * 1000.0),
            ToString(m.antiJamming.p95RecoveryTimeS * 1000.0),
            std::to_string(m.safety.maxLossBurstLength),
            ToString(m.jainFairnessIndex),
            ToString(m.jainFairnessIndex),
            ToString(m.context.coherenceTimeMs),
            m.context.channelCorrelationModel};
}
